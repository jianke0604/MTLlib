...
def main(path, lr, bs, device, ckpt_path=None):
    ...
    loss_list = []
    scores = np.array([0.0] * 3, dtype=np.float32)
    tmp_scores = np.array([0.0] * 9, dtype=np.float32)
    for epoch in epoch_iter:
        ...
        with torch.no_grad():  # operations inside don't track history
            ...
            tmp_scores = delta_fn_arr(
                avg_cost[epoch, [1, 2, 4, 5, 7, 8, 9, 10, 11]]
            )
            scores[0] = (tmp_scores[0] + tmp_scores[1])/2
            scores[1] = (tmp_scores[2] + tmp_scores[3])/2
            scores[2] = (tmp_scores[4] + tmp_scores[5] + tmp_scores[6] + tmp_scores[7] + tmp_scores[8])/5
